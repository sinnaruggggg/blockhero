...
export default function FriendsScreen({ navigation }: any) {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(`friends-screen:${playerId}`)
      .on(
        'postgres_changes',
  ...
  const handleSendRequest = async (targetId: string) => {
    ...
    setSearchResults(prev => prev.filter(result => result.id !== targetId));
  };
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.tabRow}>
        {(['friends', 'requests', 'search'] as Tab[]).map(nextTab => (
          <TouchableOpacity
            key={nextTab}
          ...
          >
            <Text
              style={[styles.tabText, tab === nextTab && styles.tabTextActive]}
            >
              {nextTab === 'friends'
              ...
              <View style={styles.friendRow}>
                ...
                <View style={styles.friendMeta}>
                  ...
                  <Text
                    style={[
                      styles.friendStatus,
                      item.isOnline && styles.friendStatusOnline,
                    ]}
                  >
      ...
      {tab === 'requests' &&
        (requests.length === 0 ? (
          ...
          <FlatList
            ...
            renderItem={({ item }) => (
              <View style={styles.friendRow}>
                <Text style={[styles.friendName, { flex: 1 }]}>
                  {item.nickname}
                </Text>
                <TouchableOpacity
